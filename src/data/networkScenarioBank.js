...
function scenario({id,week,title,requesterId,mode,priority="Medium",categories,description,inquiryLimit=2,responses,readiness,instructorNotes,knowledgeRefs}) {
  return {id,courseId:"net",week,title,requesterId,mode,priority,categories,description,inquiryLimit,clientResponses:clientResponses(responses),readiness,instructorNotes:`${instructorNotes}\nSCOPE BOUNDARY: VLAN and static-route configuration are not assessed or authorized.`,knowledgeRefs};
}
...
export const NETWORK_FINAL_SCENARIOS=finalVariants.map(([variant,subnet,fault],index)=>scenario({
  ...
  responses:{scope:"Use one router LAN interface, both switches as assigned by the pod card, and the designated laptops.",timing_change:{response:"The pod was returned from another training rotation; its exact last change is not documented.",quality:"ambiguous"},symptom_error:"One assigned laptop cannot reach its configured gateway; no more specific error was reported.",environment_equipment:`Use pod ${index+1}, the assigned console leads, tested patch cables, laptops, and approved subnet card.`,prior_troubleshooting:{response:"The previous group says they rebooted equipment and tried an address from a neighboring pod.",quality:"mistaken"},impact_urgency:"The pod must pass instructor acceptance before the next rotation can begin."},
